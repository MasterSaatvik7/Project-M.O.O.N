...
class CombinedDashboard:
    def __init__(self, root):
        ...
        self.video_source = 0  # Use default camera (can be changed to specific camera index)
        self.stream = None
        self.frame = None
    ...
    def create_camera_widgets(self):
        ...
        # Add command input controls under camera
        self.input_frame = tk.Frame(self.left_frame, bg=self.bg)
        self.input_frame.pack(fill=tk.X, pady=5)
        
        self.input_var = tk.StringVar()
        self.input_box = tk.Entry(self.input_frame, textvariable=self.input_var, font=("Arial", 12), width=30, bg=self.bg, fg=self.fg)
        ...
        btn_frame = tk.Frame(self.left_frame, bg=self.bg)
        btn_frame.pack(fill=tk.X, pady=5)
        
        motion_cmds = ["Forward", "Left", "Right", "U-Turn"]
        for idx, cmd in enumerate(motion_cmds):
            btn = tk.Button(btn_frame, text=cmd, width=10, font=("Arial", 10), bg=self.button_bg, fg=self.fg,
                          command=lambda c=cmd: self.set_input_command(c))
    ...
    def send_command(self):
        ...
        if command and self.serial_port and self.serial_port.is_open:
            ...
            print(f"Command not sent - Serial port not available: {command}")
    ...
    def get_serial_data(self):
        try:
            if self.serial_port and self.serial_port.is_open and self.serial_port.in_waiting:
                ...
                try:
                    # Assuming the data format is a comma-separated list:
                    # gyroX,gyroY,gyroZ,accX,accY,accZ,magX,magY,magZ,temperature,humidity,heading,pressure,altitude,lat,long,satellites,hdop
                    parts = [float(x) if x.strip() and not x.isalpha() else x for x in line.split(",")]
                    
                    if len(parts) >= 18:  # We expect 18 values in our data string
                        gyro = parts[0:3]
                        acc = parts[3:6]
                        mag = parts[6:9]
                        temperature = parts[9]
                        ...
                        self.lat = parts[14]
                        self.long = parts[15]
                        self.num_satellites = int(parts[16])
                        self.hdop = parts[17]
                        
                        return gyro, acc, mag, temperature, humidity, heading, pressure, altitude
                except Exception as e:
    ...
    def update_sensor_data(self):
        ...
        gyro, acc, mag, temperature, humidity, heading, pressure, altitude = self.get_serial_data()
        
        # If no data available from serial port, generate simulated data
        if all(v is None for v in [gyro, acc, mag, temperature]):
